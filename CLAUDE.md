...
- `BrowserRouter` + `vercel.json` handles routing: an explicit `/night/:slug` → `/night/:slug.html` rewrite serves the prerendered pages, then a catch-all `/((?!_vercel/).*)` → `/index.html` is the SPA fallback. The `_vercel` exclusion is required: without it the catch-all swallows `/_vercel/insights/script.js` and Web Analytics silently records nothing. Do **not** add `cleanUrls: true` alongside the catch-all — on Vercel they conflict and the catch-all silently stops firing (every non-file path 404s). Do not reintroduce the GitHub Pages `404.html` trick either.
