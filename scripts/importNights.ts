...
function mapWeekday(dayStr: string): { weekday: Weekday | null; note?: string } {
  if (!dayStr || dayStr.toLowerCase() === 'unspecified') return { weekday: null }
  const parts = dayStr.split(',').map((s) => s.trim().toLowerCase())
  const first = parts[0]
  const weekday = WEEKDAY_MAP[first] ?? null
  const note = parts.length > 1 ? `Also: ${parts.slice(1).join(', ')}` : undefined
  return { weekday, note }
}
...
for (const row of rows) {
  ...
  const { weekday, note: weekdayNote } = mapWeekday(row['Weekday / Month'])
  const rawTime = (row['Event Time'] ?? '').trim()
  const startTime = rawTime && rawTime.toLowerCase() !== 'tbc' ? rawTime : '20:00'

  const schedule: Record<string, unknown> = { frequency, startTime }
  if (weekday !== null) schedule.weekday = weekday
  else schedule.weekday = null
  if (weekdayNote) schedule.note = weekdayNote
