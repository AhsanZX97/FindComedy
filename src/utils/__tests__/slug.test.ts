...
describe('nightSlug', () => {
  it('combines the night name and venue area into a keyword-rich slug', () => {
    expect(nightSlug(makeNight())).toBe('comedy-virgins-stockwell')
  })

  it('falls back to the name alone when the area is empty', () => {
    expect(nightSlug(makeNight({ venue: { ...makeNight().venue, area: '' } }))).toBe('comedy-virgins')
  })
  ...
  it('joins multi-word names and areas with hyphens', () => {
    ...
    expect(nightSlug(night)).toBe('downstairs-at-the-king-s-head-crouch-end')
  })
})
